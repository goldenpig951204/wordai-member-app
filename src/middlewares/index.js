...
const keywordMiddleware = (prefix) => createProxyMiddleware({
    target: `https://www.keywordrevealer.com`,
    selfHandleResponse: true,
    changeOrigin: true,
    onProxyReq: (proxyReq, req) => {
        let userAgent = req.headers["user-agent"];
    ...
    onProxyRes: responseInterceptor(
        async (responseBuffer, proxyRes, req, res) => {
            ...
            if (proxyRes.headers["location"]) {
                proxyRes.headers["location"] = proxyRes.headers["location"].replace("https://www.keywordrevealer.com", domain);
                res.setHeader("location", proxyRes.headers["location"].replace("https://www.keywordrevealer.com", domain));
            }
            if (
                proxyRes.headers["content-type"] && 
                proxyRes.headers["content-type"].includes("text/html")
    ...
    hostRewrite: true,
    headers: {
        referer: "https://www.keywordrevealer.com",
        origin: "https://www.keywordrevealer.com"
    },
    autoRewrite: true,
    ws: true
});
    ...
    if (proxy !== null) {
        if (setting != null) {
            ...
            if (proxy.type == "semrush") {
                ...
                return keywordMiddleware(prefix)(req, res, next);
            }
        } else {
